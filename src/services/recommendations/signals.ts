    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed))
          events = parsed.filter(isValidEvent).slice(-MAX_EVENTS);
      }
    } catch {
      /* corrupted store: start empty, never crash the app */
    }
    loaded = true;
  })();
  return loading;
...
function persist(): void {
  ...
  saveTimer = setTimeout(() => {
    saveTimer = null;
    getKV()
      .set(STORAGE_KEY, JSON.stringify(events.slice(-MAX_EVENTS)))
      .catch(() => {
...
export function recordComplete(track: {
  id: string;
  artist: string;
  title: string;
}): void {
...
export function recordSkip(
  ...
  if (!track?.id) return;
  const fraction = Number.isFinite(fractionPlayed)
    ? Math.max(0, Math.min(1, fractionPlayed))
    : 0;
  if (fraction >= 0.9) {
...
export function recordPlaylistAdd(track: {
  id: string;
  artist: string;
  title: string;
}): void {
  ...
  append({
    at: Date.now(),
    type: "playlist_add" as SignalType,
    trackId: track.id,
    artist: track.artist,
    title: track.title,
  });

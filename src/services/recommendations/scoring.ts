...
export const WEIGHTS = {
  ...
  explorationMax: 0.3, // hard upper bound of the exploration share
...
export function trackAffinity(stats: {
  plays: number;
  completes: number;
  liked: boolean;
  playlistAdds: number;
  ...
  const positive =
    (stats.liked ? WEIGHTS.like : 0) +
    stats.completes * WEIGHTS.completion +
    stats.plays * WEIGHTS.play +
    stats.playlistAdds * WEIGHTS.playlistAdd;
  const negative =
    stats.skips * Math.abs(WEIGHTS.skip) +
    stats.quickSkips * Math.abs(WEIGHTS.quickSkip);
...
export function artistAffinity(
  stats: Pick<
    ArtistStats,
    "plays" | "completes" | "likes" | "skips" | "quickSkips" | "playlistAdds"
  >,
): number {
  return Math.max(
    0,
...
export function scoreCandidate(
  ...
  if (artistStats)
    score +=
      WEIGHTS.artistAffinity *
      (context.maxArtistAffinity > 0
        ? artistAffinity(artistStats) / context.maxArtistAffinity
        : 0);
  if (trackStats)
    score +=
      WEIGHTS.trackAffinity *
      (trackAffinity(trackStats) / WEIGHTS.maxTrackWeight);
  ...
  if (candidate.source === "radio" || candidate.source === "likes-radio") {
    score +=
      WEIGHTS.seedStrength * Math.pow(WEIGHTS.seedDecay, candidate.seedRank);
    const seedStats = profile.trackStats.get(candidate.track.id);
    score +=
      WEIGHTS.recency * recencyFactor(seedStats?.lastPlayedAt ?? null, now);
  }

  // Recent search interest: candidate artist or source label matches a searched term.
  if (
    profile.searchTerms.some(
      (entry) =>
        candidate.artistKey.includes(entry.term) ||
        entry.term.includes(candidate.artistKey),
    )
  ) {
  ...
  if (
    !trackStats &&
    !artistStats &&
    !profile.historyIds.has(candidate.track.id)
  )
  ...
  score += deterministicJitter(
    `${candidate.track.id}:${profile.version}`,
    WEIGHTS.jitter,
  );
  return score;
}

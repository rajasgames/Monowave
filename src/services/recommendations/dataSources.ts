...
export async function radioCandidates(
  seed: Track,
  profile: TasteProfile,
  seedRank: number,
): Promise<Candidate[]> {
  const cacheKey = `radio:${seed.id}`;
  let payload = getCached<{
    tracks: Track[];
  ...
  if (!payload) {
    ...
    if (!payload.tracks.length)
      throw new Error(`No related tracks for seed ${seed.id}`);
  ...
  return payload.tracks.map((track) =>
    toCandidate(track, profile, "radio", seed.title, seedRank),
  );
}

/** Radio for liked seeds with day-stable rotation, merged into one pool. */
export async function likedRadioCandidates(
  profile: TasteProfile,
): Promise<Candidate[]> {
  ...
  const settled = await Promise.allSettled(
    chosen.map(async (seed, index) => {
      ...
      return payload.tracks.map((track) =>
        toCandidate(track, profile, "likes-radio", seed.title, index),
      );
    }),
  );
...
export async function resolveArtistId(
  ...
  matches.sort(
    (a, b) => Number(toKey(b.title) === key) - Number(toKey(a.title) === key),
  );
...
export async function artistCandidates(
  ...
  if (!tracks) {
    ...
    if (!tracks.length)
      throw new Error(`No tracks on artist page ${artistName}`);
    putCached(cacheKey, tracks);
  }
  return tracks.map((track) =>
    toCandidate(track, profile, "artist", artistName, 0),
  );
}

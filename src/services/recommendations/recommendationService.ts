...
import { applyDiversity, applyExplorationQuota, interleavePools } from './diversity';
import { artistCandidates, likedRadioCandidates, radioCandidates, resolveArtistId, searchCandidates } from './dataSources';
...
import type { Candidate, LibrarySnapshot, RecoResult, RecoSection, TasteProfile } from './types';
...
async function pool<T>(jobs: (() => Promise<T>)[], concurrency: number): Promise<PromiseSettledResult<T>[]> {
...
function scoreAll(candidates: Candidate[], profile: TasteProfile): Candidate[] {
  const maxArtistAffinity = profile.topArtists.length ? artistAffinity(profile.topArtists[0]) : 0;
  const now = Date.now();
  return candidates
    .map(candidate => ({ ...candidate, score: scoreCandidate(candidate, { profile, seedRank: candidate.seedRank, maxArtistAffinity, now }) }))
    .sort((a, b) => b.score - a.score);
}

/** Ids to keep out of "new music" surfaces: the tail of recent listening. */
function recentHistoryIds(library: LibrarySnapshot, count: number): Set<string> {
...
function buildRecentlyPlayed(library: LibrarySnapshot): RecoSection | null {
  ...
  return { kind: 'recently-played', title: 'Recently played',
    subtitle: `${library.history.length} ${library.history.length === 1 ? 'listen' : 'listens'}`, tracks };
}

function buildRediscover(profile: TasteProfile, library: LibrarySnapshot): RecoSection | null {
  ...
  for (const [id, stats] of profile.trackStats) {
    if (stats.plays > 1 && !stats.liked && !seen.has(id)) {
      const track = library.history.find(entry => entry.track.id === id)?.track;
  ...
  return { kind: 'rediscover', title: 'Rediscover', subtitle: 'Strong picks you have not played lately', tracks: picks };
}
 ...
 */
async function buildBecauseYouPlayed(profile: TasteProfile, exclude: Set<string>): Promise<{ section: RecoSection | null; pool: Candidate[] }> {
  const seeds = profile.seedTracks.slice(0, 2);
  if (!seeds.length) return { section: null, pool: [] };
  const settled = await Promise.allSettled(seeds.map(async (seed, rank) => ({
    seed,
...
async function buildBasedOnLikes(profile: TasteProfile, exclude: Set<string>): Promise<{ section: RecoSection | null; pool: Candidate[] }> {
...
async function buildMoreFromArtist(profile: TasteProfile, exclude: Set<string>): Promise<RecoSection | null> {
  ...
  try {
    ...
    const diversified = applyDiversity(ranked, { excludeIds: exclude, maxPerArtist: WEIGHTS.sectionSize, windowMax: WEIGHTS.sectionSize });
    const picked = diversified.slice(0, WEIGHTS.sectionSize);
 ...
 */
async function buildDiscoverMix(profile: TasteProfile, library: LibrarySnapshot, pools: Candidate[][], ignoreMixCache: boolean): Promise<{ section: RecoSection | null; pool: Candidate[] }> {
  if (profile.strongSignalCount < WEIGHTS.coldStartStrongSignals) return { section: null, pool: [] };
  ...
  for (const { term } of profile.searchTerms.slice(0, 2)) {
    try { allPools.push(scoreAll(await searchCandidates(term, profile), profile)); }
    catch { /* search pool is optional */ }
  }
  if (!allPools.length) return { section: null, pool: [] };

  const ranked = scoreAll(interleavePools(allPools.filter(list => list.length)), profile);
  ...
  const diversified = applyDiversity(ranked, { excludeIds: exclude, seedIds, maxPerArtist: WEIGHTS.mixPerArtist });
  if (diversified.length < WEIGHTS.mixAbsoluteMin) return { section: null, pool: diversified };

  const cacheKey = `mix:${profile.version}`;
  const cachedMix = ignoreMixCache ? null : getCached<string[]>(cacheKey, TTL.mix);
  let mixTracks: Track[] = [];
  const byId = new Map(diversified.map(candidate => [candidate.track.id, candidate]));
  if (cachedMix?.length) {
    mixTracks = cachedMix.map(id => byId.get(id)?.track).filter((track): track is Track => Boolean(track));
  }
  if (mixTracks.length < WEIGHTS.mixAbsoluteMin) {
    const targetSize = Math.min(WEIGHTS.mixMax, Math.max(WEIGHTS.mixMin, Math.round(diversified.length * 0.85)));
    const { picked } = applyExplorationQuota(diversified, targetSize);
    mixTracks = picked.map(candidate => candidate.track);
    if (mixTracks.length >= WEIGHTS.mixAbsoluteMin) putCached(cacheKey, mixTracks.map(track => track.id), WEIGHTS.mixMax);
  }
  if (mixTracks.length < WEIGHTS.mixAbsoluteMin) {
  ...
  const explorationCount = mixTracks.filter(track => byId.get(track.id)?.exploration).length;
  return {
    section: {
      kind: 'discover-mix',
      title: 'Discover Mix',
      ...
      tracks: mixTracks,
      context: explorationCount ? `${explorationCount} newer picks included` : undefined,
    },
    pool: diversified,
  };
}
...
export async function generateRecommendations(library: LibrarySnapshot, options: GenerateOptions = {}): Promise<RecoResult> {
  ...
  if (!library.history.length && !library.liked.length && !signals.length) {
    return {
      status: 'cold', sections: [],
      notes: ['Cold start: nothing listened to yet. Play, finish, or like a few tracks to build real recommendations.'],
      builtAt: Date.now(),
    };
  }
  ...
  const settledRadios = await pool(profile.seedTracks.slice(0, 4).map((seed, rank) => () => radioCandidates(seed, profile, rank)), 2);
  settledRadios.forEach((result, index) => {
    if (result.status === 'fulfilled') seedPools.push(result.value);
    else notes.push(`Radio for seed "${profile.seedTracks[index]?.title ?? '?'}" failed: ${String(result.reason).slice(0, 120)}`);
  });
  ...
  const settledArtists = await pool(profile.topArtists.slice(0, 4).map(artist => async () => {
    const browseId = await resolveArtistId(artist.name);
    if (!browseId) throw new Error(`no artist page for ${artist.name}`);
    return artistCandidates(artist.name, browseId, profile);
  }), 2);
  settledArtists.forEach(result => { if (result.status === 'fulfilled') artistPools.push(result.value); });

  const mix = await buildDiscoverMix(
    profile, library,
    [...seedPools, ...artistPools, ...(because.pool.length ? [because.pool] : []), ...(likes.pool.length ? [likes.pool] : [])],
    Boolean(options.ignoreMixCache),
  );
  if (!mix.section && profile.strongSignalCount < WEIGHTS.coldStartStrongSignals) {
    notes.push(`Discover Mix waits for ${WEIGHTS.coldStartStrongSignals} strong signals (likes + full listens + repeats); currently ${profile.strongSignalCount}.`);
  } else if (!mix.section && mix.pool.length) {
    notes.push('Discover Mix skipped: not enough distinct real candidates right now.');
  }

  // Fixed presentation order; every slot optional.
  const byKind = new Map<RecoSection['kind'], RecoSection>();
  for (const section of [mix.section, because.section, moreFrom, likes.section, ...local]) {
  ...
  if (!ordered.length) notes.push('No section had enough real data to display yet.');

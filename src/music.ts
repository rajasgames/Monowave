...
async function request(endpoint: string, body: Record<string, unknown>): Promise<any> {
...
function runs(value: any): string {
  return (value?.runs ?? []).map((entry: any) => entry?.text ?? '').join('').trim();
}
function thumb(value: any): string | undefined {
  const choices = value?.musicThumbnailRenderer?.thumbnail?.thumbnails ?? value?.thumbnails ?? value?.thumbnail?.thumbnails ?? [];
...
function row(renderer: any): SearchItem | null {
  const title = runs(renderer?.flexColumns?.[0]?.musicResponsiveListItemFlexColumnRenderer?.text);
  const secondary = renderer?.flexColumns?.slice(1)
    .map((column: any) => runs(column?.musicResponsiveListItemFlexColumnRenderer?.text))
    .filter(Boolean).join(' · ') ?? '';
  ...
  if (videoId) {
    const artist = secondary.split(' • ').find((x: string) => x && !/^song$|^video$/i.test(x)) || 'Unknown artist';
    const track: Track = { id: videoId, title, artist, cover, duration: seconds(secondary.split(' • ').at(-1) ?? '') };
    return { id: videoId, title, subtitle: secondary || artist, cover, kind: 'track', track };
  }
  if (browseId) {
    const kind = browseId.startsWith('UC') ? 'artist' : browseId.startsWith('MPRE') ? 'album' : 'playlist';
...
function card(renderer: any): SearchItem | null {
  ...
  return { id, title, subtitle, cover: thumb(renderer?.thumbnailRenderer),
    kind: id.startsWith('UC') ? 'artist' : id.startsWith('MPRE') ? 'album' : 'playlist' };
}

function extract(json: any): SearchItem[] {
  const items = [...findRenderers(json, 'musicResponsiveListItemRenderer').map(row),
    ...findRenderers(json, 'musicTwoRowItemRenderer').map(card),
    ...findRenderers(json, 'playlistPanelVideoRenderer').map((renderer: any): SearchItem | null => {
      const id = renderer?.videoId;
      const title = runs(renderer?.title);
      if (!id || !title) return null;
      const artist = runs(renderer?.shortBylineText) || runs(renderer?.longBylineText) || 'Unknown artist';
      const track: Track = { id, title, artist, cover: thumb(renderer?.thumbnail), duration: seconds(runs(renderer?.lengthText)) };
      return { id, title, subtitle: artist, cover: track.cover, kind: 'track', track };
    })]
...
export async function browseMusic(id: string): Promise<SearchItem[]> {
  const response = await request('browse', { browseId: id.startsWith('PL') ? `VL${id}` : id });
...
export async function trackById(id: string): Promise<Track> {
  ...
  return extract(response).find(item => item.kind === 'track' && item.id === id)?.track ??
    { id, title: `YouTube track ${id}`, artist: 'Unknown artist' };
}
 ...
 */
export async function fetchRadioTracks(videoId: string): Promise<{ tracks: Track[]; artistIds: Record<string, string> }> {
  const response = await request('next', { videoId, playlistId: `RDAMVM${videoId}`, isAudioOnly: true });
  const tracks = extract(response)
    .filter(item => item.kind === 'track' && item.track && item.id !== videoId)
    .map(item => item.track!);
  const artistIds: Record<string, string> = {};
  for (const renderer of findRenderers(response, 'playlistPanelVideoRenderer')) {
    for (const run of renderer?.longBylineText?.runs ?? []) {
      const browseId = run?.navigationEndpoint?.browseEndpoint?.browseId;
      const name = typeof run?.text === 'string' ? run.text.trim() : '';
      if (browseId?.startsWith('UC') && name && !artistIds[name]) artistIds[name] = browseId;
    }
  }
...
export async function fetchArtistTracks(browseId: string): Promise<Track[]> {
  ...
  return items.filter(item => item.kind === 'track' && item.track).map(item => item.track!);
}

export function parseLink(input: string): { kind: 'track' | 'playlist'; id: string } | null {
  try {
    const url = new URL(input.trim());
    if (!['music.youtube.com', 'www.youtube.com', 'youtube.com'].includes(url.hostname)) return null;
    const list = url.searchParams.get('list');
    if (list && /^[A-Za-z0-9_-]+$/.test(list)) return { kind: 'playlist', id: list };
    const id = url.searchParams.get('v');
    if (id && /^[A-Za-z0-9_-]{11}$/.test(id)) return { kind: 'track', id };
  } catch { /* Input is not a URL. */ }
  return null;
}
